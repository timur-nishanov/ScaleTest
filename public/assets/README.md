...
**Статус (25.08):** разложено из заливки заказчика — шрифты (ttf + сгенерированные woff2), 18 иконок сервисов (переименованы в ключи), 3 фигуры заставки (`illustrations/shape-main-*.svg`).
**Не хватает:** `logo/yc-logotype.svg` (на заставке пока текстовый плейсхолдер — подхватится автоматически), иллюстрации карточек задач (`kv_*`), иллюстрации попапов, композиции карточек режимов.
