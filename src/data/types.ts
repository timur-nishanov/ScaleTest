...
export interface BuildTask {
  ...
  title: string
  /** Полное условие задачи — шапка экрана сборки (показывается целиком). */
  cardDesc: string
  /** Короткое описание для карточки-превью в колесе (140–150 символов,
   *  тексты лида YC от 10.09); если не задано — берётся cardDesc. */
  previewDesc?: string
...
export interface ReadyTask {
  ...
  title: string
  cardDesc: string
  /** Короткое описание для карточки-превью; если не задано — cardDesc. */
  previewDesc?: string
  assignment: string
